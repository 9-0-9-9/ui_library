...
function Library:CreateWindow(name)
    ...
    UserInputService.InputChanged:Connect(function(input) if input == dragInput and Window.Dragging then update(input) end end)
    local function UpdateHeight()
        local contentSize = Layout.AbsoluteContentSize
        if not Window.Dragging then Tween(Main, TweenInfo.new(0.1), {Size = UDim2.new(0, 300, 0, math.clamp(contentSize.Y + 25, 25, 800))}) end
    end
    ...
    function Window:AddTab(name)
        ...
        local function toggle()
            ...
            if Tab.IsOpen and targetH <= 25 then
                local calcH = 25
                for _, c in ipairs(Tab.Children) do calcH = calcH + (c.Frame.Size.Y.Offset) end
                targetH = calcH
            end
        ...
        function Tab:AddToggle(name, default, callback)
            ...
            TLabel.Size, TLabel.Position, TLabel.BackgroundTransparency, TLabel.Text, TLabel.TextColor3, TLabel.TextSize, TLabel.Font, TLabel.TextXAlignment, TLabel.Parent = UDim2.new(0.5, -10, 1, 0), UDim2.new(0, 30, 0, 0), 1, name, Color3.fromRGB(150, 150, 150), 13, Enum.Font.RobotoMono, Enum.TextXAlignment.Left, TFrame
        ...
        function Tab:AddSlider(name, default, callback)
            ...
            SLabel.Size, SLabel.Position, SLabel.BackgroundTransparency, SLabel.Text, SLabel.TextColor3, SLabel.TextSize, SLabel.Font, SLabel.TextXAlignment, SLabel.Parent = UDim2.new(0.5, -10, 1, 0), UDim2.new(0, 30, 0, 0), 1, name, Color3.fromRGB(150, 150, 150), 13, Enum.Font.RobotoMono, Enum.TextXAlignment.Left, SFrame
        ...
        function Tab:AddDropdown(name, options, callback)
            ...
            DLabel.Size, DLabel.Position, DLabel.BackgroundTransparency, DLabel.Text, DLabel.TextColor3, DLabel.TextSize, DLabel.Font, DLabel.TextXAlignment, DLabel.Parent = UDim2.new(1, -10, 0, 25), UDim2.new(0, 30, 0, 0), 1, "+ " .. name, Color3.fromRGB(150, 150, 150), 13, Enum.Font.RobotoMono, Enum.TextXAlignment.Left, DFrame
